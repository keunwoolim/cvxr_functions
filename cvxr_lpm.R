...
lpm_problem <- function(.x, type = c("minrisk","optimal"),
                       expected_reward = rep(0, ncol(.x)),
                       lower = rep(0, ncol(.x)),
                       upper = rep(1, ncol(.x)),
                       target = 0, budget = NULL, leverage = NULL,
                       solver = NULL, verbose = FALSE, moment = 1, threshold = 999, ...)
{
  m <- NCOL(.x)
  n <- NROW(.x)
  onematrix <- matrix(1, n, 1)
...
#      z <- Variable(m, boolean = TRUE)
  ...
  w <- as.numeric(w)
  port_risk <- riskfun(as.matrix(w), .x, risk = "ev")
...
cvxr_solution <- lpm_problem(.x, type = c("minrisk","optimal")[2], 
                                 expected_reward = colMeans(.x) * 10000,
                                 lower = rep(-0.2, ncol(.x)), 
                                 upper = rep(0.4, ncol(.x)), 
                                 budget = NULL, leverage = 0.5, solver = "GLPK_MI")
...
cvxr_solution <- lpm_problem(.x, type = c("minrisk","optimal")[1], 
                                 expected_reward = colMeans(.x) * 10000,
                                 lower = rep(-0.8, ncol(.x)), 
                                 upper = rep(0.8, ncol(.x)), 
                                 target = 2,
                                 budget = NULL, leverage = 1, solver = "GLPK_MI")


spec <- parmaspec(scenario = .x, forecast = colMeans(.x) * 10000, 
                  risk = "LPM", riskType = "minrisk",
...
cvxr_solution <- lpm_problem(.x, type = c("minrisk","optimal")[1], 
                                 ...
                                 upper = rep(0.8, ncol(.x)), 
                                 target = 2,
                                 budget = 1, leverage = NULL, solver = "GLPK_MI")
...
# optimal solution with higher moments
cvxr_solution <- lpm_problem(.x, type = c("minrisk","optimal")[2],
                             expected_reward = colMeans(.x) * 10000,
                             lower = rep(0, ncol(.x)),
                             upper = rep(0.8, ncol(.x)),
                             budget = 1, leverage = NULL, solver = "SCS", moment = 2)


spec <- parmaspec(scenario = .x, forecast = colMeans(.x) * 10000, 
                  risk = "LPM", riskType = "optimal",
                  LB = rep(0, 4), UB = rep(0.8, 4), target = 0, budget = 1, moment = 2)
parma_solution <- parmasolve(spec)
