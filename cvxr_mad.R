...
mad_problem <- function(.x, type = c("minrisk","optimal"),
...
#      z <- Variable(m, boolean = TRUE)
  ...
  x_normal <-  as.matrix(sweep(.x, 2, t(colMeans(.x)), "-"))
  risk <- CVXR::norm1(x_normal %*% w)
  ...
  w <- as.numeric(w)
  port_risk <- riskfun(as.matrix(w), .x, risk = "mad")
...
cvxr_solution <- mad_problem(.x, type = c("minrisk","optimal")[2], 
...
cvxr_solution <- mad_problem(.x, type = c("minrisk","optimal")[1], 
                                 expected_reward = colMeans(.x) * 10000,
