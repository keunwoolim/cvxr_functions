...
print(cov(.x))
minimax_problem <- function(.x, 
                            type = c("minrisk","optimal"), 
                            expected_reward = rep(0, ncol(.x)), 
                            lower = rep(0, ncol(.x)), 
                            upper = rep(1, ncol(.x)), 
                            target = 0, budget = NULL, leverage = NULL, 
                            solver = NULL, verbose = FALSE, ...)
{
    # turn it into a minimization problem
    .x <- -.x
    # rescale expected reward
    n <- NCOL(.x)
    if (type == "minrisk") {
        w <- Variable(n)
        if (is.null(budget) & !is.null(leverage)) {
            # exact 1-norm problem
            w_positive <- Variable(n)
            w_negative <- Variable(n)
            z <- Variable(n, boolean = TRUE)
            ones <- rep(1, n)
        ...
        risk <- CVXR::max_entries(.x %*% w)
        admissible_solver <- c("GLPK_MI", "ECOS_BB")
    } else {
        w <- Variable(n)
        ...
        risk <- CVXR::max_entries(.x %*% w)
        admissible_solver <- c("CBC","GLPK_MI", "ECOS_BB")
    }
    ...
    names(w) <- colnames(.x)
    return(list(weights = w, risk = port_risk, reward = port_reward, risk_type = "minimax", problem_type = type))
}


# optimal solution with leverage
cvxr_solution <- minimax_problem(.x, type = c("minrisk","optimal")[2], 
                            expected_reward = colMeans(.x)*10000,
                            lower = rep(-0.2, ncol(.x)), 
                            upper = rep(0.4, ncol(.x)), 
                            budget = NULL, leverage = 0.5, solver = "CBC")


spec <- parmaspec(scenario = .x, forecast = colMeans(.x), 
                  risk = "MiniMax", riskType = "optimal",
                  LB = rep(-0.2,4), UB = rep(0.4, 4), leverage = 0.5)
...
cvxr_solution <- minimax_problem(.x, type = c("minrisk","optimal")[1], 
                                 expected_reward = colMeans(.x)*10000,
                                 lower = rep(-0.8, ncol(.x)), 
                                 upper = rep(0.8, ncol(.x)), 
                                 target = 2,
                                 budget = NULL, leverage = 1, solver = "CBC")


spec <- parmaspec(scenario = .x, forecast = colMeans(.x) * 10000, 
                  risk = "MiniMax", riskType = "minrisk",
                  target = 2, targetType = "inequality",
                  LB = rep(-0.8,4), UB = rep(0.8, 4), leverage = 1)
parma_solution <- parmasolve(spec)
